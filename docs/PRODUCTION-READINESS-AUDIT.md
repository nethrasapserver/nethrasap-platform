...
| **CR-3** | **Committed deploy blueprint boots to a crash.** `ENVIRONMENT=production` + `SMS_PROVIDER=console` hits the config guard → hard boot fail; `STORAGE_*` unset does the same; `.onrender.com` base breaks guest carts/refresh; `NEXT_PUBLIC_DASHBOARD_URL` unset → localhost link. | — | `render.yaml`, `infra/Dockerfile.*` | ◑ **Code hardened (Gate 2)** — non-root images, multi-stage, /docs gated, render.yaml amended, worker cmd fixed; **deploy provisioning pending accounts** |
| **CR-4** | **Product search effectively unusable.** Whole-word only; no prefix/fuzzy; multi-word → 0. | `amox→0, para→0, "blood pressure"→0, "amoxicillin 500"→0` | `services/catalogue.py` | ✅ **Fixed (Gate 3)** — prefix + multi-word to_tsquery |
| **CR-5** | **Role pricing wrong on listings — price shown ≠ charged.** Verified retailer browses retail prices on cards but is charged wholesale. | listing ₹2,116.80 vs PDP/cart ₹1,848 | `services/catalogue.py` (backend) | ◑ **Backend fixed+verified (Gate 3)**; storefront SSR list still base-prices logged-in wholesale buyers (PDP/cart correct) — UI follow-up |
...
|---|---|---|---|
| **H-1** | Unauthenticated order-book enumeration — omit `phone_last4` → any order's data; sequential numbers → whole book scrapeable; no rate limit. | `services/orders.py`, `api/v1/orders.py` | ✅ **Fixed (Gate 2)** — fail-closed + auth-gate + rate-limit |
| **H-2** | Nested-JSON DoS crashes every POST (RecursionError → 500), unauth; no request-body size cap. | `main.py`, `middleware.py` | ✅ **Fixed (Gate 2)** — body-size cap + recursion-safe handler |
...
| **H-5** | No COD order can ever be refunded, and no record COD cash was collected (`captured_at` stays NULL forever). | fulfilment refund requires `captured`; delivery never captures COD | ✅ **Fixed (Gate 3)** — COD-collect action captures cash + unlocks refund |
...
| **H-12** | Dashboard can't add a 2nd pack size — the "…" menu opens the drawer over itself; clicks fire no request. | `dashboard products/page.tsx` z-order | ✅ **Fixed (Gate 3)** — menu stopPropagation |
| **H-13** | A 403 renders as legitimate zero-data — manager sees an empty warehouse (0/0/0) against 22 SKUs / 4,791 units. | inventory read gated on `inventory:write`; 403 not surfaced | ✅ **Fixed (Gate 3)** — 403 renders access-denied, not zeros |
...
| **H-16** | Header ⌘K search never submits — Enter fires no request; only preset chips work; no autocomplete. | storefront header search | ✅ **Fixed (Gate 3)** — ⌘K submits to /products?q= |
| **H-17** | Order page hangs forever on an unknown order number — API 404, no error state. | `orders/[orderNumber]` | ✅ **Fixed (Gate 3)** — not-found card, no infinite spinner |
| **H-18** | Cart accepts quantities above available stock, no warning (checkout-place enforcement to re-verify). | storefront cart / PDP stepper | ◑ Checkout enforces (Gate 1 FOR UPDATE); cart-qty UX clamp = minor follow-up |
| **H-19** | Test/seed junk live in the customer catalogue — "xyz", "NoBrand Test Tonic" (₹0), "AUDIT-99 tablets" purchasable. | seed / audit test data | ✅ Resolved by reseed (clean seed catalogue) |
| **H-20** | Unmoderated reviews with no proof-of-purchase — a payload review dragged Amoxicillin to 1.0 (React escapes → no XSS exec). | `services/reviews.py:74` | ✅ **Fixed (Gate 3)** — verified-purchase gate |
| **H-21** | OTP entry doesn't auto-advance — typing/pasting a 6-digit code fills only box 1; signup/OTP login unusable. | storefront OTP inputs | ✅ Source already correct (packages/ui) — stale build artifact; rebuilt |
| **H-22** | Role prices silently dropped when MRP is blank (success toast, tiers lost); `/track` has no order-number lookup despite 3 CTAs. | dashboard price editor; storefront `/track` | ✅ **Fixed (Gate 3)** — /track lookup + blank-MRP validation |
