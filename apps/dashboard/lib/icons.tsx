...
function make(parts: Part[]): React.FC<IconProps> {
  return function Icon(props: IconProps = {}) {
    const size = props.size ?? 18;
    const stroke = props.stroke ?? 1.7;
    const className = props.className ?? "";
    const style = props.style;
    const children = parts.map((p, i) => React.createElement(p[0], { key: i, ...p[1] }));
...
const P = (d: string): Part => ["path", { d }];
const C = (x: number, y: number, r: number): Part => ["circle", { cx: x, cy: y, r }];
const R = (x: number, y: number, w: number, h: number, rx?: number): Part => ["rect", { x, y, width: w, height: h, rx }];
...
export const Icons = {
  ...
  Edit:        make([P("M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"), P("M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5Z")]),
...
export const resolveHomeIcon = (name: string): React.FC<IconProps> =>
  (Icons as any)[name] || Icons.Sparkle;
